...
const unsigned int RANDOM_DIRECTION_TIMER_MIN_DELAY = 1000;
...
uint8_t default_scroll_timer;
uint8_t extra_scroll_timer;
...
void setup() {
  ...
  default_scroll_timer = scheduler.setInterval(SCROLL_SPEED, scrollDisplays);
  scheduler.setInterval(UPDATE_TURBO_STATUS_INTERVAL, updateTurboStatus);
  scheduler.setTimeout(STARTUP_ANIMATION_DURATION, startGame);
  // scheduler.setInterval(300, updatePlayerStatus);

  extra_scroll_timer = scheduler.setInterval(SCROLL_SPEED_FAST, scrollDisplays);
  scheduler.disable(extra_scroll_timer);

  random_speed_timer = scheduler.setInterval(RANDOM_SPEED_TIMER, randomSpeed);
  random_direction_timer =
      scheduler.setInterval(RANDOM_DIRECTION_TIMER, randomDirection);
}
...
void randomDirection() {
  ...
  if (motor_direction == MOTOR_FORWARD && random(0, 100) > RANDOM_STOP) {
    ...
    scheduler.setTimeout(
        random(RANDOM_DIRECTION_TIMER_MIN_DELAY, RANDOM_DIRECTION_TIMER),
        randomDirection);
  } else if (motor_direction == MOTOR_FORWARD &&
             random(0, 100) > DIRECTION_RANDOM) {
...
void disableButtons() { BUTTONS_ENABLED = false; }
...
void enableExtraScrollTimer() {
  scheduler.disable(default_scroll_timer);
  scheduler.enable(extra_scroll_timer);
}

void disableExtraScrollTimer() {
  scheduler.enable(default_scroll_timer);
  scheduler.disable(extra_scroll_timer);
}
...
void looserFound(int p) {
  ...
  scheduler.setTimeout(LOOSER_ANIMATION_DURATION, startGame);
...
void readInputs() {
  ...
  for (int p = 0; p < DISPLAY_COUNT; p++) {
    ...
    if (BUTTONS_ENABLED && one_player_active && game_state == GAME_IDLE &&
      ...
      enableExtraScrollTimer();
      return;
    } else if (BUTTONS_ENABLED && game_state == GAME_ACTIVE &&
               player_status[p] == CHIP_ACTIVE && (~player_input & B00000010)) {
      // TURBO
    ...
    } else if (game_state == GAME_ACTIVE && player_status[p] != CHIP_INACTIVE &&
               (player_input & B00000001)) {
      // LICHTSCHRANKE
      ...
      player_status[p] = CHIP_ACTIVE;
    } else if (game_state == GAME_CHECK_PLAYERS &&
               (~player_input & B00000001)) {
      player_status[p] = CHIP_ACTIVE;
